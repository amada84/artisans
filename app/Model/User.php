...
class User extends AppModel {
	...
	public $validate = array(
		'username' => array(
			'required' => array(
				...
				'message' => 'Vueillez entrer un pseudo.'),
			'alpha' => array(
		...
		'pack_id' => array(
			'rule' 		=> 'notEmptyPack',
			'message'	=> 'Vous devez choisir un pack')
	);
	...
	public function notEmptyPack($pack = null){		
		if($this->data[$this->alias]['type'] == 'pro')
			return $pack['pack_id'] != 0;
		return true;
	}
	...
	public function edit($userId = null, $postData = null) {
		...
		if (!empty($postData)) {
			...
			if ($user[$this->alias]['type'] != $postData[$this->alias]['type']) {
				if ($postData[$this->alias]['type'] == 'pro') {
					
				} elseif ($postData[$this->alias]['type'] == 'par') {
	...
	public function changeStatus($postData = array()) {
		$type = $postData[$this->alias]['type'];
		if ($type == 'pro') {
			$this->PackUser = ClassRegistry::init('PackUser');
			$this->PackUser->add($postData[$this->alias]['pack_id'], $postData[$this->alias]['id']);
		} elseif ($type == 'par') {
			$this->PackUser = ClassRegistry::init('PackUser');
			$thi->PackUser->deleteStatus($postData[$this->alias]['pack_id'], $postData[$this->alias]['id']);
		}
	}

	public function add_credit($postData = null) {
		if (!empty($postData)) {
			$this->data = $postData;
			if (1 == 1) {
				$this->set($postData);
	...
	public function lastCredit($userId = null) {
		$this->PackUser = ClassRegistry::init('PackUser');
		return $this->PackUser->find('first', array(
			'conditions' => array(
				'PackUser.user_id' => $userId,
			)
		));
	}
	...
	public function view($id = null, $conditions = array()) {
		$default_conditions = array(
			'conditions' => array(
				$this->alias . '.' . $this->primaryKey => $id,
			)
		);
		$user = $this->find('first', array_merge($default_conditions, $conditions));
		if (empty($user)) {
	...
	public function afterFind($results, $primary = false) {
		foreach ($results as $k => $v) {
			if (isset($v[$this->alias]['id']) && isset($v[$this->alias]['slug'])) {
				...
				if ($v[$this->alias]['type'] == 'pro') {
					$this->PackUser = ClassRegistry::init('PackUser');
					$v[$this->alias]['credit'] = current($this->PackUser->find('all', array(
						'conditions' => array(
							'user_id' => $v[$this->alias]['id']
						),
					)));
				} else {
					$v[$this->alias]['credit'] = array();
				}			
			}
			$results[$k] = $v;
		}
...
/* Partie supplémentaire à revoir au besoin */
	...
	public function resendVerification($postData = array()) {
		...
		if ($user[$this->alias]['active'] == 0) {
			$this->invalidate('email', __d('users', 'Votre compte est .'));
			return false;
		}
