...
class UsersController extends AppController {
	...
	public function register() {
		...
		$packs = $this->Pack->find('list');
		array_unshift($packs, 'Choisissez un pack');
		$types = Configure::read('Users.types');
		$this->set(compact('packs', 'types'));
	}
		...
		if ($this->request->is('post')) {
			if ($this->Auth->login()) {
				...
				if($this->Auth->user('role') == 'admin'){
					...
					$this->Auth->loginRedirect = array('admin' => false, 'controller' => 'articles', 'action' => 'index');
				}

				$this->Session->setFlash(sprintf(__d('users', '%s vous êtes connectés.'), $this->Auth->user('username') . ' ' .$this->Auth->user('lastname')), 'notif');
	...
	public function admin_index() {
		...
		$this->Paginator->settings[$this->modelClass]['conditions'] = array_merge($parsedConditions, array($this->modelClass . '.active' => 1, $this->modelClass . '.role !=' => 'admin', $this->modelClass . '.is_admin !=' => 1));

		$this->User->recursive = 0;
		$users = $this->Paginator->paginate();
		$types = Configure::read('Users.type');
		$actives = Configure::read('Users.active');
		$this->set(compact('users', 'types', 'actives'));
	}

	public function admin_list_pro() {
		$this->Prg->commonProcess();
		unset($this->User->validate['username']);
		unset($this->User->validate['email']);
		unset($this->User->validate['type']);
		unset($this->User->validate['active']);
		...
		$this->Paginator->settings[$this->modelClass]['conditions'] = array_merge($parsedConditions, array($this->modelClass . '.active' => 1, $this->modelClass . '.role !=' => 'admin', $this->modelClass . '.is_admin !=' => 1, $this->modelClass . '.type' => 'pro'));
	...
	public function admin_list_par() {
		$this->Prg->commonProcess();
		unset($this->User->validate['username']);
		unset($this->User->validate['email']);
		$this->User->data[$this->modelClass] = $this->passedArgs;
		...
		$this->Paginator->settings[$this->modelClass]['conditions'] = array_merge($parsedConditions, array($this->modelClass . '.active' => 1, $this->modelClass . '.role !=' => 'admin', $this->modelClass . '.is_admin !=' => 1, $this->modelClass . '.type' => 'par'));
	...
	public function admin_inactive_users() {
		$this->Prg->commonProcess();
		unset($this->User->validate['username']);
		unset($this->User->validate['email']);
		unset($this->User->validate['type']);
		...
		$this->Paginator->settings[$this->modelClass]['conditions'] = array_merge($parsedConditions, array($this->modelClass . '.active' => 0, $this->modelClass . '.role !=' => 'admin', $this->modelClass . '.is_admin !=' => 1));
	...
	public function admin_list_admins() {
		$this->Prg->commonProcess();
		unset($this->User->validate['username']);
		unset($this->User->validate['email']);
		$this->User->data[$this->modelClass] = $this->passedArgs;
		...
		$this->Paginator->settings[$this->modelClass]['conditions'] = array_merge($parsedConditions, array($this->modelClass . '.role' => 'admin', $this->modelClass . '.is_admin' => 1));
	...
	public function admin_view($id = null) {
		try {
			...
			$this->Session->setFlash(__d('users', 'Utlisateur non valide.'));
			$this->redirect(array('action' => 'index'));
		}
		$types = Configure::read('Users.types');
		$this->set(compact('types', 'user'));
	}
	...
	public function admin_edit($userId = null) {
		...
		$this->set('types', Configure::read('Users.types'));
	}

	public function admin_add_credit() {
		if (!empty($this->request->data)) {
			if ($this->User->add_credit($this->request->data)) {
				$this->Session->setFlash(__d('users', 'L\'utilisateur a été sauvegardé.'), 'notif');
				$this->redirect($this->referer());
			}
		}
	...
	public function admin_edit_admin($userId = null) {
		try {			
			$result = $this->User->edit($userId, $this->request->data);
			if ($result === true) {
				$this->Session->setFlash(__d('users', 'L\'utilisateur a été sauvegardé.'), 'notif');
